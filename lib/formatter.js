...
function formatAnchor(elem, fn, options) {
	/**
   * By Thanh Nguyen, 20/2/2014
   * Custom options.noLink, to allow htmlToText get the <a> contentText instead of href value
   * Add a space before it
   * @TODO add a space after it in cases
   */
  if (options.noLink === true) {
    return ' ' + fn(elem.children, options);
  } else if (elem.attribs && elem.attribs.href) {
